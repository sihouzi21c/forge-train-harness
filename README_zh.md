...
> **工作区隔离说明**：每次 `bash agent-loop.sh` 都会自动在
> `.artifacts/forge_train/<loop_id>/workspace/` 下复制一份 `harness/`
> 作为本次循环的隔离工作区，源码树不会被改动。Agent 每轮的 commit
> 都落在该工作区里；要查看、提取或对比某次 loop 生成的引擎代码，
> 去 `.artifacts/forge_train/<loop_id>/workspace/workload/src/` 下找。
...
agent-loop.sh starts
 ...
 └─────────────────────────────────────────────────────┘
      │
      ▼
 产出物: <workspace>/workload/src/training_engine_tensor/
         （完整可训练的 LLM 训练引擎，
          其中 <workspace> = .artifacts/forge_train/<loop_id>/workspace/）
```

#### Outputs

运行完成后，本次 loop 的所有产出都挂在 `.artifacts/forge_train/<loop_id>/`
下：


| 路径（相对 `.artifacts/forge_train/<loop_id>/`）          | 说明                                                                              |
| --------------------------------------------------------- | ------------------------------------------------------------------------------- |
| `workspace/workload/src/training_engine_tensor/`          | 生成的训练引擎源码（forward / backward / kernels / optimizer / dataloader / train_loop 等） |
| `workspace/workload/notes/perf_log.md`                    | 每轮迭代的性能日志                                                                       |
| `workspace/.artifacts/agent-logs/<时间戳>/`                | 完整的 agent 对话日志与 review 记录                                                       |
| `session.json`                                            | 本次 loop 的注册表记录（status / pid / args / 路径）                                       |
